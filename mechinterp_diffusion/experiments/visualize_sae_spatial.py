...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot SAE Activations")
    parser.add_argument(
        "--model_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to load activations from cache. "
        "If set to False, it will recompute and overwrite the cache.",
    )
    ...
    # indices = [i for i, ts in enumerate(test_dataset["timestep"]) if ts == 1]
...
class SpatialVisualizer:
    def __init__(
        ...
        self.dataset.set_format(
            type="torch", columns=["activations", "timestep"]
        )
    ...
    def run(self) -> None:
        ...
        time_data, features_data = self._process_samples()
    ...
    def _plot_aggregated_patterns(
        ...
        """Create overall aggregated spatial pattern."""

        timesteps = sorted(data.keys())
        ic(data[timesteps[0]]["spatial_mean"].shape)

        # Aggregate over ALL timesteps: mean across time
        time_aggregated = np.stack(
            [data[t]["spatial_mean"] for t in timesteps]
        ).mean(
            axis=0
        )  # [h, w]

        lo, hi = np.percentile(time_aggregated, (2, 98))

        ic(time_aggregated.shape)

        fig = plt.figure(figsize=(8, 6))
        gs = GridSpec(1, 1, hspace=0.2, wspace=0.3)

        # Raw aggregated pattern only (no smoothing)
        ...
        cbar1.set_label("Activation Strength", fontsize=10)

        save_path = self.output_dir / "spatial_aggregated_pattern.pdf"
    ...
    def _process_samples(
        ...
        for timestep in tqdm(self.timesteps, desc="Evaluating timesteps"):
            ...
            for batch_idx, batch in enumerate(test_loader):
                ...
                ic(sae_acts_spatial.shape)

                # [h, w]
                spatial_mean = sae_acts_spatial.mean(dim=(0, -1)).cpu().numpy()
                spatial_nonzeros = (
                    (sae_acts_spatial > 0.0).sum(dim=(0, -1)).cpu().numpy()
                )
                ...
                if data[timestep]["count"] == 0:
                    ...
                    data[timestep]["spatial_mean"] = (
                        data[timestep]["spatial_mean"] * n + spatial_mean
                    ) / (n + bs)
                ...
                feature_stats["sum_sq"] += (feature_np**2).sum(
                    axis=1
                )  # [d_sae, h, w]
        ...
        with h5py.File(self.cache_file, "w") as f:
            for t in self.timesteps:
                ...
                group.create_dataset("mean", data=data[t]["spatial_mean"])
            ...
            features_group.create_dataset(
                "spatial_mean", data=features_data["spatial_mean"]
            )
            features_group.create_dataset(
                "spatial_variance", data=features_data["spatial_variance"]
            )
            features_group.create_dataset(
                "mean_activation", data=features_data["mean_activation"]
            )
            features_group.create_dataset(
                "feature_ranking", data=features_data["feature_ranking"]
            )
